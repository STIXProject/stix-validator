...
class SchematronValidator(object):
    ...
    def _init_schematron(self, schematron):
        ...
        if schematron is None:
            ...
            return
        elif not (isinstance(schematron, etree._Element) or
                  isinstance(schematron, etree._ElementTree)):
    ...
    def _build_error_dict(self, errors, instance_tree, report_line_numbers=True):
        ...
        if report_line_numbers:
            for d_error in d_errors.itervalues():
                line_numbers = self._get_error_line_numbers(d_error,
                                                            instance_tree)
                d_error['line_numbers'] = line_numbers
        
        return d_errors

    def _build_error_report_dict(self, validation_report, instance_tree,
    ...
    def validate(self, instance, report_line_numbers=True):
        ...
        Arguments:
        report_line_numbers : Includes error line numbers in the returned
                              dictionary. This may slow performance.
                              
        '''
        if not self.schematron:
            raise Exception('Schematron document not set. Cannot validate. '
                            'Call init_schematron(...) and retry.')
        try:
