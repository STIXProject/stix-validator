...
def print_best_practice_results(fn, results):
    if results['result']:
        ...
        print_level("[!] Best Practice validation results: %s : INVALID",
                    0, fn)
...
def convert_profile(validator, xslt_out_fn=None, schematron_out_fn=None):
    ...
    if schematron_out_fn:
        info("Writing schematron conversion of profile to %s" %
             schematron_out_fn)
...
def main():
    ...
    parser.add_argument("--input-file", dest="infile", default=None,
                        help="Path to STIX instance document to validate")
    parser.add_argument("--input-dir", dest="indir", default=None,
                        help="Path to directory containing STIX instance "
        ...
        if schema_validation:
            ...
            if len(to_validate) > 0:
                ...
                for fn in to_validate:
                    ...
                    info("Validating STIX document %s against XML schema... "
                         % fn)
                    schema_results = schema_validator.validate(fn,
                                     version=args.stix_version,
                                    schemaloc=args.use_schemaloc)
                    isvalid = schema_results['result']

                    if args.best_practices:
                        if isvalid:
                            best_practice_results = bp_validator.validate(fn,
                                                    args.stix_version)
                        else:
                    ...
                    if profile_validation:
                        if isvalid:
                            info("Validating STIX document %s against profile "
                                 "%s..." % (fn, args.profile))
                            profile_results = profile_validator.validate(fn)
                        else:
                            info("The document %s was schema-invalid. Skipping "
                                 "profile validation" % fn)
                    ...
                    if args.json:
                        ...
                        if best_practice_results:
                            print_best_practice_results(fn,
                                                        best_practice_results)
                        if profile_results:
                            print_profile_results(fn, profile_results)

        if profile_conversion:
            convert_profile(profile_validator, xslt_out_fn=args.xslt,
                            schematron_out_fn=args.schematron)
